...
<body>
    ...
    <div>
        <div class="table-list" style="position: -webkit-sticky;position: sticky;top: 0; margin-bottom: 10px;">
          <h2>Lista de Tablas</h2>
          
          <div id="tableList" style="display: flex; flex-wrap: wrap; flex-direction: row;">
            <?php
              $scriptPath = __FILE__;
              ...
              while ($table = $tables->fetchArray()) {
                  ...
                  echo "<button onclick='showTable(\"$tableName\")'>$tableName</button>";
              }
    ...
    <div id="tableData" style="overflow-y: auto;" ></div>
    ...
    <script>
        ...
        async function removeRecordsCheked() {
          if(tableNameSelected == "posted_products"){
            // Copia de los registros seleccionados para no modificar el array original
            let records = [...SELECTED_RECORDS];

            while (records.length) {
              // Toma 10 registros de la lista
              const batch = records.splice(0, 10);
              console.log("Eliminando lote:", batch);

              // Ejecuta removeRecord para cada registro del lote en paralelo y espera a que terminen
              await Promise.all(
                batch.map(async (v) => {
                  console.log("removeRecord", v.tableName, v.id);
                  await removeRecord(v.tableName, v.id);
                  // Opcional: Eliminar la fila del DOM, si aplica
                  const fila = document.getElementById(`id-tr-${v.id}`);
                  if (fila) {
              ...
              console.log("Lote eliminado. Esperando 2 segundos antes del siguiente lote...");
              // Espera 2 segundos antes de procesar el siguiente lote
